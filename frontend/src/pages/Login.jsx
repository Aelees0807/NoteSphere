...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Get our custom JWT from the backend
      const res = await axios.post(`${API_URL}/auth/login`, { email: user.email });
      localStorage.setItem('token', res.data.token);
      navigate('/');
    } catch (err) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="w-full max-w-md p-8 space-y-4 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 shadow-lg">
        <h2 className="text-3xl font-bold text-center text-charcoal-blue dark:text-white">Welcome Back</h2>
        ...
        <p className="text-sm text-center text-gray-700 dark:text-gray-300">
            ...
            <Link to="/register" className="font-medium text-electric-blue hover:underline">Register Now</Link>
        </p>
      </div>
    </div>
  );
